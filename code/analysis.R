...
# datanovia.com/en/lessons/cohens-kappa-in-r-for-two-categorical-variables/
...
############################## Prevalence ##############################
## POPULATION: 5778 eyes of 3821 unique participants had high myopia
...
female  <- sample[!duplicated(sample$id) & sample$sex=="Female",]
N_cases <- sum(female$PMbinary_individualLevel)
N_all   <- nrow(female)
tmp     <- as.matrix(cbind(N_cases, N_all))
epi.conf(tmp, 
         ctype      = "prevalence", 
...
# Prevalence in males
female  <- sample[!duplicated(sample$id) & sample$sex=="Male",]
N_cases <- sum(female$PMbinary_individualLevel)
N_all   <- nrow(female)
tmp     <- as.matrix(cbind(N_cases, N_all))
epi.conf(tmp, 
...
resultPlaceholder                                            <-  printPrevalence(sample, sample$SERgroup)
...
resultPlaceholder                                            <-  printPrevalence(sample, sample$ageGroup)
...
sample$ethnic                <- ifelse(sample$ethnic == "Indian" | sample$ethnic == "Pakistani", "Indian/Pakistani", sample$ethnic)                      # Break down non-White ethnicity and get prevalence
resultPlaceholder            <-  printPrevalence(sample, sample$ethnic) 
...
## PM prevalance in different SER by OD orientation groups
